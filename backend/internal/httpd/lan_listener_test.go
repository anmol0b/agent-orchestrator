...
func TestLANManagerAuthGatesSharedHandler(t *testing.T) {
	...
	m := NewLANManager(inner, st, 0, slog.Default(), nil, nil) // port 0 → ephemeral
	port, err := m.Start(0)
	if err != nil {
...
func TestLANManagerBlocksLoopbackOnlyControlRoutes(t *testing.T) {
	...
	st := &authState{}
	st.setHash(mobilebridge.HashPassword("secret12"))
	m := NewLANManager(inner, st, 0, slog.Default(), nil, nil)
	port, err := m.Start(0)
	if err != nil {
		t.Fatalf("start: %v", err)
	}
...
func TestLANManagerStartStopIdempotent(t *testing.T) {
	m := NewLANManager(http.NotFoundHandler(), &authState{}, 0, slog.Default(), nil, nil)
	p1, _ := m.Start(0)
